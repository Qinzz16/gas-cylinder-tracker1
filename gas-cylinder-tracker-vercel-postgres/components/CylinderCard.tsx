...
import { quickTurnOff, quickTurnOn } from "@/lib/quick-actions";
...
export function CylinderCard({ cylinder }: CylinderCardProps) {
  ...
  return <article className={`cylinder-card ${cylinder.isOn ? "on" : ""} ${cylinder.currentStatus === "ALMOST_EMPTY" ? "almost" : ""}`}>
    ...
    <div className="actions">
      <form action={cylinder.isOn ? quickTurnOff : quickTurnOn}>
        <input type="hidden" name="cylinderId" value={cylinder.id}/>
        <button className={`btn ${cylinder.isOn ? "btn-off" : "btn-on"}`} type="submit">{cylinder.isOn ? "Turn OFF" : "Turn ON"}</button>
      </form>
    ...
    <details className="disclosure"><summary>Activity timeline ▾</summary>
      <div className="timeline">{cylinder.events.length ? cylinder.events.map((event) => <div className="timeline-item" key={event.id}><strong>{prettyEnum(event.eventType)}</strong><time>{displayDate(event.eventTime)}</time></div>) : <span className="muted small">No activity yet.</span>}</div>
    </details>
  </article>;
}
